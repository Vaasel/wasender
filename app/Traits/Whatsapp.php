...
trait Whatsapp
{
    ...
    private function getChats($device_id)
    {
        ...
        if ($status != 200) {
            ...
            $contacts = $colllections->map(function ($item) {
                ...
                if (isset($item->conversationTimestamp)) {
                    if (is_numeric($item->conversationTimestamp)) {
                        $data['timestamp'] =(int)  $item->conversationTimestamp;
                    } else {
                        $data['timestamp'] =(int)  $item->conversationTimestamp->low;
                    }
                } else if (!empty($item->messages) && is_array($item->messages)) {
                    $data['timestamp'] = (int) $item->messages[0]->message->messageTimestamp ?? 0;
                }else{
                    $data['timestamp'] = 0;
                }
            ...
            $sortedContacts = $contacts->sortByDesc('timestamp')->take(500);
            $sortedContactsArray = $sortedContacts->values()->all();


            $responseData['status'] = 200;
            $responseData['data'] = $sortedContactsArray;
        }
    ...
    public function getMessages($chat_id, $device_id)
    {
        $device_id = 'device_' . $device_id;
        $whatsServer = env('WA_SERVER_URL');

        $url = $whatsServer . '/chats/getmessages/' . $chat_id . '?id=' . $device_id;
    ...
    public function getContactList($device_id)
    {
        $session_id = 'device_' . $device_id;
        $whatsServer = env('WA_SERVER_URL');

        $response = Http::get($whatsServer . '/chats/contacts?id=' . $session_id);
        $status = $response->status();
    ...
    private function saveFile(Request $request, $input)
    {
        ...
        $urlWithoutStorage = str_replace('storage/', '', $url);
    
        return $urlWithoutStorage;
    }
