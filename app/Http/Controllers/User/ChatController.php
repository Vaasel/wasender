...
$filter = true;
        ...
        if ($request->selecttype == 'media-text') {
            $validated = $request->validate([
    ...
    public function getChatMessages(Request $request)
    {
        ...
        $messages = Cache::remember(
            "messages_" . $device->uuid . $request->id,
            120,
            function () use ($device, $request) {
                return $this->getMessages($request->id, $device->id);
            }
        );
    ...
    public function contacts($id)
    {
        ...
        $response = Cache::remember(
            "contacts_" . $device->uuid,
            120,
            function () use ($device) {
                return $this->getContactList($device->id);
            }
        );
        if ($response["status"] == 200) {
            $data = $response;
