...
function downloadCsvFile(csvContent,name) {
...
$('#downloadCsv').on('click', function () {
	...
	$.each(_groupdata.participants, function (index, item) {
		const wa_number = item.id.replace('@s.whatsapp.net', '')
		jsonData.push({
			name: _contacts[item.id]?.name || _contacts[item.id]?.notify || 'wa ' + wa_number ,
			number : wa_number ,
		})
	})

    var csvContent = convertJsonToCsv(jsonData)
    downloadCsvFile(csvContent,_groupdata.subject + '.csv');
})
